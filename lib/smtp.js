...
// 🔁 Counter rotasi global
let currentIndex = 0;

// 📋 Ambil semua akun Gmail dari environment variables terpisah
function getGmailAccounts() {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    console.log(`[SMTP DEBUG] ${envKey} = "${accountStr}"`);
    ...
    if (email && password) {
      ...
      console.log(`[SMTP] ✅ Akun #${i} dimuat: ${email} (password: ${'*'.repeat(password.length)})`);
    } else {
...
// 🔄 Ambil akun berikutnya secara rotasi
function getNextAccount() {
  ...
  console.log(`[SMTP] 🔄 Rotasi: Index ke-${currentIndex} dari ${accounts.length} akun`);
  
  const account = accounts[currentIndex];
  currentIndex = (currentIndex + 1) % accounts.length;
  
  console.log(`[SMTP] 📧 Mengirim dari: ${account.email} (Index berikutnya: ${currentIndex})`);
